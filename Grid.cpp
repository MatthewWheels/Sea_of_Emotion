...
//  Sea_of_Emotion
//
//  Created by Matthew Wheeler on 2/13/20.
//
    ...
    noiseTime+=0.01+_waveSpeed;
            ...
            for(int j =0; j < 300; j++){
                ...
                //Sets the vertex based upon the U(pdate)Index value and the brightness of the pixel color previously generated
                    grid.setVertex(UIndex, glm::vec3(j * 20+(myNoise*_ampX)-1000, _baseY+(myNoise*_ampY), _z*20+(myNoise*2)));
             ...
             if(myNoise >=0){
                ...
                //sets the vertex and the color of the next vertex in the index
                 grid.setVertex((UIndex), glm::vec3(j * 20+(myNoise*_ampX)-1000, _baseY+(myNoise*_ampY), (_z+1)*20+(myNoise*2)));
                 grid.setColor(UIndex,oceanColor);
